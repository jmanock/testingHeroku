...
var dilberFeedURL = 'http://feeds.feedburner.com/DilbertDailyStrip';
...
function addEmptyFeedListTest(callback){
  ...
  .get(tc.url + '/feeds').auth(user.sp_api_key_id, user.sp_api_key_secret)
  .expectStatus(200).expectHeader('Content-Type', 'application/json; charset=utf-8')
  .expectJSON({feeds:[]}).toss();
  callback(null);
}
function subOneFeed(callback){
  ...
  .expectStatus(200).expectHeader('Content-Type', 'application/json; charset=utf-8')
  .expectJSONLength('user.subs', 1).toss();
  callback(null);
}
function subDuplicateFeed(callback){
  ...
  .expectStatus(201).expectHeader('Content-Type', 'application/json; charset=utf-8')
  .expectJSONLength('user.subs', 1).toss();
  callback(null);
}
function subSecondFeed(callback){
  ...
  .expectStatus(201).expectHeader('Content-Type', 'application/json; charset=utf-8')
  .expectJSONLength('user.subs', 2).toss();
  callback(null);
}
...
async.series([addEmptyFeedListTest, subOneFeed, subDuplicateFeed, subScondFeed, subOneFeedScondUser]);
